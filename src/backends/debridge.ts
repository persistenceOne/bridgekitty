...
export class DeBridgeBackend implements BridgeBackend {
  ...
  async buildTransaction(quote: BridgeQuote): Promise<TransactionRequest> {
    ...
    if (isSolanaChain(p.srcChainId)) {
      ...
      if (!serializedTx) {
        throw new Error(
          "Invalid Solana transaction data in deBridge create-tx response. " +
  ...
  async getStatus(
    ...
    try {
      ...
      return {
        state: "unknown",
        humanReadable: `Status check failed: ${sanitizeError(err as Error)}. If you just submitted the transaction, wait 1-2 minutes for the bridge provider to index it.`,
        provider: "debridge",
        elapsed: 0,
      };
    }
