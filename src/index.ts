...
function createEngine(): RoutingEngine {
  ...
  const lifi = new LiFiBackend(
    process.env.LIFI_API_KEY, // user can provide their own LI.FI API key for higher rate limits
 ...
 */
async function startMcpServer(engine: RoutingEngine): Promise<void> {
 ...
 */
async function startAcpListener(engine: RoutingEngine): Promise<void> {
