...
function validateQuoteParams(params: QuoteParams): void {
  ...
  if (params.fromChainId === params.toChainId &&
    ...
    throw new BackendValidationError(
      `Source and destination tokens are the same on chain ${params.fromChainId}. Nothing to swap.`
    );
  }
  ...
  if (fromIsSolana && !/^[1-9A-HJ-NP-Za-km-z]{32,44}$/.test(params.fromAddress)) {
    throw new BackendValidationError(
      `Invalid Solana sender address: "${params.fromAddress}". Expected base58-encoded address.`
    );
  }
  ...
  if (params.toAddress) {
    ...
    if (toIsSolana && !/^[1-9A-HJ-NP-Za-km-z]{32,44}$/.test(params.toAddress)) {
      throw new BackendValidationError(
        `Invalid Solana recipient address: "${params.toAddress}". Expected base58-encoded address.`
      );
    }
  }

  // Token addresses: must look like EVM addresses, Cosmos denoms, or Solana base58 mint addresses
  const isValidTokenAddress = (addr: string, chainIsCosmos: boolean, chainIsSolana: boolean): boolean => {
    if (isValidEvmAddress(addr)) return true;
    if (chainIsCosmos && /^[a-z][a-z0-9/]{1,128}$/.test(addr)) return true;
    if (chainIsSolana && /^[1-9A-HJ-NP-Za-km-z]{32,44}$/.test(addr)) return true;
    return false;
  };
...
export class RoutingEngine {
  ...
  private lastFailedProviders: FailedProvider[] = [];
  ...
  async getQuotes(params: QuoteParams): Promise<CachedQuote[]> {
    ...
    for (let i = 0; i < results.length; i++) {
      ...
      if (r.status === "rejected") {
        ...
        if (errMsg.includes("timed out")) reason = `timeout after ${BACKEND_TIMEOUT_MS / 1000}s`;
        else if (errMsg.includes("rate limit") || errMsg.includes("429")) reason = "rate limited";
        else if (errMsg.includes("ECONNREFUSED") || errMsg.includes("ENOTFOUND")) reason = "connection failed";
        else reason = errMsg.slice(0, 100);
