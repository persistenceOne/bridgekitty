...
function getExplorerTxUrl(chainIdOrQuoteData: number | any, txHash: string): string {
    ...
    async (params) => {
      ...
      try {
        ...
        if (params.sign_and_send) {
            ...
            return {
              content: [{
                ...
                text: JSON.stringify({
                  ...
                  explorerUrl: getExplorerTxUrl(quote.quoteData as any, result.txHash),
          ...
          if (isSolanaChain(quote.quoteData && typeof quote.quoteData === "object" && "srcChainId" in (quote.quoteData as any) ? (quote.quoteData as any).srcChainId : 0)) {
          ...
          if (txRequest.approvalTx) {
            ...
            if (txRequest.needsPostApprovalBuild) {
              const freshTx = await withTimeout(
                backend.buildTransaction(quote),
                BUILD_TX_TIMEOUT_MS,
                `buildTransaction post-approval (${backend.name})`
              );
